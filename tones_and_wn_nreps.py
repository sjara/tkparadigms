'''
Present a range of stimuli (pure tones and white noise) logarithmically spaced
and at different intensities. Specify number of reps for each stim condition

This is paradigm that presents both whitenoise and pure tones at different frequencies and intensities. As a cheat, we added a "-1" freq to the toneList and added an if statement in PrepareNextTrial that sets the stimtype to noise if the freq is -1.  

Computes number of trials from numReps and numFreqs and numInt, and calls dispatcher gui to stop when you reach that number of trials
'''
...
class Paradigm(QtWidgets.QMainWindow):
    def __init__(self, parent=None, paramfile=None, paramdictname=None):
        ...
        self.params['numReps'] = paramgui.NumericParam('Number of repetitions',
                                                        value=20, 
                                                        group='Stim parameters')
        self.params['numTrials'] = paramgui.NumericParam('Total number of trials',
        ...
        self.params['numTrials'].set_value(self.params['numReps'].get_value() * (self.params['numTones'].get_value() + 1) * self.params['numInt'].get_value())  
    ...
    def populate_sound_params(self):
        ...
        toneList = np.append(toneList, -1) # Add noise to the list of frequencies. this is where we added the "-1" freq 
        ...
        productList = list(itertools.product(toneList, ampList)) ## TODO: maybe add noise to this list somehow 
        ...
        self.params['numTrials'].set_value(self.params['numReps'].get_value() * (numFreqs+1) * numInt)
    ...
    def prepare_next_trial(self, nextTrial):
        ...
        if nextTrial >=self.params['numTrials'].get_value():
        ...
        if self.trialParams[0] == -1: ## this is where the modification is made. If the freq is -1, then the stimType is noise 
            stimType = 'Noise'
