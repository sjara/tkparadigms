...
A cue is played before subject initiates target presentation. Subject has to discriminate if
target was of higher or lower frequency than the cue.
...
class Paradigm(templates.Paradigm2AFC):
    def __init__(self,parent=None, paramfile=None, paramdictname=None):
        ...
        self.params['delayToTargetMean'] = paramgui.NumericParam('Mean delay to target',value=0.1,
        ...
        self.params['punishTimeEarly'] = paramgui.NumericParam('Punishment (early)',value=0,
                                                        units='s',group='Timing parameters')
        self.params['cueDuration'] = paramgui.NumericParam('Cue duration',value=0.1,
                                                        units='s',group='Timing parameters')
        ...
        self.params['trialsPerBlock'] = paramgui.NumericParam('Trials per block',value=3000,
    ...
    def prepare_sounds(self,targetFrequency,cueFrequency):
        ...
        targetDur = self.params['targetDuration'].get_value()
        s1 = {'type':'chord', 'frequency':targetFrequency, 'duration':targetDur,
              'amplitude':targetAmp, 'ntones':12, 'factor':1.2}
        self.soundClient.set_sound(1,s1)

        ###cueFrequency = self.params['midFreq'].get_value()
        cueIntensity = self.params['cueIntensity'].get_value()
        cueAmp = spkCal.find_amplitude(cueFrequency,cueIntensity).mean()
        #self.params['cueAmplitude'].set_value(cueAmp)
        cueDur = self.params['cueDuration'].get_value()
        s2 = {'type':'chord', 'frequency':cueFrequency, 'duration':cueDur,
              'amplitude':cueAmp, 'ntones':12, 'factor':1.2}
        self.soundClient.set_sound(2,s2)
    ...
    def set_state_matrix(self,nextCorrectChoice):
        ...
        targetSoundID = 1  # FIXME: hardcoded (defined in prepare_sounds)
        cueSoundID = 2  # FIXME: hardcoded (defined in prepare_sounds)
        targetDuration = self.params['targetDuration'].get_value()
        cueDuration = self.params['cueDuration'].get_value()
        cueInterval = self.params['cueInterval'].get_value()
        ...
        if outcomeMode=='simulated':
            ...
            self.sm.add_state(name='delayPeriod', statetimer=delayToTarget,
                              transitions={'Tup':'playTarget','Cout':'waitForCenterPoke'})
            self.sm.add_state(name='playTarget', statetimer=targetDuration,
            ...
            if outcomeModeString=='on_next_correct' or outcomeModeString=='only_if_correct':
                ...
                if self.sm.statesNameToIndex['reward'] in eventsThisTrial[:,2]:
                   ...
                   if outcomeModeString=='on_next_correct' and \
                    ...
                    # Note that outcome='nochoice' is already set when testing for choice
                    pass
