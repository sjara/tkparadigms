...
class Paradigm(templates.Paradigm2AFC):
    def __init__(self,parent=None, paramfile=None, paramdictname=None):
        ...
        self.params['targetAmplitude'] = paramgui.NumericParam('Target amplitude avg',value=0.0,units='[0-1]',
    ...
    def prepare_target_sound(self, soundParam):
        ...
        spkCalChords = speakercalibration.Calibration(rigsettings.SPEAKER_CALIBRATION_CHORD)
        # NOTE: For some sounds calibration is averaged from both speakers. Not for noise.

        if self.params['soundType'].get_string() == 'AM':
            # print('----------------------------------')
            print(f'---- {targetIntensity} dB-SPL: {spkCalNoise.find_amplitude(targetIntensity)} ----')
            # print('----------------------------------')
            targetAmp = spkCalNoise.find_amplitude(targetIntensity)
            targetAmpMean = targetAmp.mean()
            self.params['targetAmplitude'].set_value(targetAmpMean)
            stimDur = self.params['targetDuration'].get_value()
